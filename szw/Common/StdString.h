...
//
//	  2000-MAR-07 - Thanks to Ullrich Poll锋ne for catching a range bug in one
...
inline void	ssasn(std::wstring& sDst, PCWSTR pW)
{
	...
	else if (sDst.size() > 0 && pW >= sDst.c_str() && pW <= sDst.c_str() + sDst.size() )
	{
...
public:
	...
	// They also are compiled if you have explicitly turned off refcounting
		...
		MYTYPE& assign(const MYTYPE& str, MYSIZE nStart, MYSIZE nChars)
		{
			...
			// value.  Thanks to Ullrich Poll锋ne for catching this bug
...
//		}
	...
	#define MAX_FMT_TRIES		500	 // #of times we try 
	...
	void FormatV(const CT* szFormat, va_list argList)
	{
		...
		do	
		{
			...
			//modified by hpt 使用_malloca解决/RTC1选项时，多次分配内存后函数退出异常的问题
			pBuf			= reinterpret_cast<CT*>(_malloca(sizeof(CT)*nChars));  
			...
			//modified by hpt 释放_malloca申请的内存
			if (nUsed < 0)
			{
				_freea(pBuf);
				pBuf = NULL;
...
/*		CString strTemp	= strA;*/
		CString strTemp(strA); //modify by yzh
		return ar << strTemp;
	}
