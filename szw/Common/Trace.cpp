...
#ifdef _DEBUG
	#pragma comment(lib,"ZipFunction_d.lib")
#else
	#pragma comment(lib,"ZipFunction.lib")
#endif // DEBUG
...
bool CLogTrace::IsLogFileOpened()
{
 ...
 //打开/创建日志文件
void CLogTrace::OpenLogFile(const TCHAR * pszTraceFileName, HWND hWnd, BOOL bNeedZip)
{
	TCHAR tszTmp[MAX_PATH] = {0};
	_stprintf(tszTmp, _T("%s\\data2\\mc.dat"), GetInstallPath());
	IXMLRW xmlCfg;
	xmlCfg.init(tszTmp);
	xmlCfg.ReadInt(_T("MC"), _T("DEBUGLOG"), _T("Encrypt"), m_nEncryptLog, 1);
	xmlCfg.ReadInt(_T("MC"), _T("DEBUGLOG"), _T("Output"), m_nOutLogInfo,1);
	xmlCfg.ReadInt(_T("MC"), _T("DEBUGLOG"), _T("OutLevel"), m_nOutLogLevel, 3);
	xmlCfg.ReadInt(_T("MC"), _T("DEBUGLOG"), _T("SaveAll"), m_nSaveAllLog, 0);
...
//
///////////////////////////////////////////////////////////////////////////////
CLogTrace::CLogTrace(const TCHAR * pszTraceFileName, HWND hWnd, BOOL bNeedZip)
{
	TCHAR tszTmp[MAX_PATH] = {0};
	_stprintf(tszTmp, _T("%s\\data2\\mc.dat"), GetInstallPath());
	IXMLRW xmlCfg;
