...
struct T_Limit : public CEnDeCode
{
	...
	DWORD dwSingleRegistCount;	 //每天可注册数量
	...
	CStdString Get()
	{
		...
		strTmp.Format(_T("%d"), dwSingleRegistCount);
		strData += strTmp;
	...
	void Set(CStdString &strData)
	{
		...
		if (i < nSize)
		{
			dwSingleRegistCount = (E_IeLimit)_ttoi(vecData[i++]);
		}
	}
};
...
struct T_Site_Type
{
	...
	BOOL bCloudSchedule;//网站不抢占焦点(不抢占为1，抢占焦点为0)
	BOOL bSelectAll;//是否支持全行业匹配
	BOOL bIsObject;//是不是需要单任务推广网站
...
//////////////////////////////////////////////////////////////////////////
/////////////////网站支持类型///////////////////////////////////////////////
struct T_Yun_Support : public CEnDeCode
{

	BOOL bSupportGrab;		  //是否不抢占焦点
	BOOL bSupportYunGist;	  //是否支持云推广功能
	BOOL bSupportYunPush;	  //是否支持云发布功能
	BOOL bSupportYunRefresh;  //是否支持云刷新功能
	BOOL bSupportYunDelete;   //是否支持云删除功能
	BOOL bSupportYunModefy;	  //是否支持云修改资料功能
	BOOL bSupportYunActive;	  //是否支持云激活验证功能
	...
	{
		bSupportGrab = FALSE;
		bSupportYunGist = FALSE;
		bSupportYunPush = FALSE;
		bSupportYunRefresh = FALSE;
		bSupportYunDelete = FALSE;
		bSupportYunModefy = FALSE;
		bSupportYunActive = FALSE;
	}
	...
	CStdString Get()
	{
		...
		strTmp.Format(_T("%d"), bSupportGrab);
		strData += strTmp;

		strTmp.Format(_T("%d"), bSupportYunGist);
		strData += strTmp;

		strTmp.Format(_T("%d"), bSupportYunPush);
		strData += strTmp;

		strTmp.Format(_T("%d"), bSupportYunRefresh);
		strData += strTmp;

		strTmp.Format(_T("%d"), bSupportYunDelete);
		strData += strTmp;

		strTmp.Format(_T("%d"), bSupportYunModefy);
		strData += strTmp;

		strTmp.Format(_T("%d"), bSupportYunActive);
		strData += strTmp;

		return strData;
	}

	void Set(CStdString &strSupportType)
	{
		if (strSupportType.IsEmpty())
		{
			return;
		}

		int nLen = strSupportType.GetLength();
		int i = 0;
		if (i < nLen)
		{
			bSupportGrab = _ttoi(strSupportType.Mid(i++, 1));
		}
		if (i < nLen)
		{
			bSupportYunGist = _ttoi(strSupportType.Mid(i++, 1));
		}
		if (i < nLen)
		{
			bSupportYunPush = _ttoi(strSupportType.Mid(i++, 1));
		}
		if (i < nLen)
		{
			bSupportYunRefresh = _ttoi(strSupportType.Mid(i++, 1));
		}
		if (i < nLen)
		{
			bSupportYunDelete = _ttoi(strSupportType.Mid(i++, 1));
		}
		if (i < nLen)
		{
			bSupportYunModefy = _ttoi(strSupportType.Mid(i++, 1));
		}
		if (i < nLen)
		{
			bSupportYunActive = _ttoi(strSupportType.Mid(i++, 1));
		}
	}
};
...
struct T_Station_Info : public CEnDeCode
{
	...
	T_Yun_Support  tYunSupprot; //网站相关云推广支持结构体
	...
	void Set(CStdString &strData)
	{
		...
		if (i < nSize)
		{
			tYunSupprot.Set(vecStationInfo[i++]);
		}
	}
};
