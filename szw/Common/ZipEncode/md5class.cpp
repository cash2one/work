...
void CMD5::setBinaryData(unsigned char* binarydata,int dwLen)
{
        //
        m_binary = const_cast<unsigned char*>(binarydata);
        dwDataLen = dwLen;
        m_digestValid = calcDigestB();
...
bool CMD5::calcDigestB()
{
  ...
  //the alternative to these ugly casts is to go into the RFC code and change the declarations
  MD5Update(&context, reinterpret_cast<unsigned char *>(m_binary), dwDataLen);
  MD5Final(reinterpret_cast <unsigned char *>(m_digest),&context);
