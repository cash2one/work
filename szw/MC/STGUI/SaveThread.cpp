...
DWORD CSaveThread::SaveResultToGui( DWORD dwIndex )
{
	...
	{
		strTaskId.Format(_T("%d"), pTaskData->tTaskNetItem.dwTaskId);
	}

	//返回的Mac地址
	CStdString strMac = _T("");
	strMac = pLocalGlobalData->szMac;
	//写入返回给GUI的结果
	...
	case ETypeRefresh:
		{
			...
			strPostGui.Format(_T("%s%s%s%s%d%s%s%s%s%s%s%s%s"), strPostGui, 
	...
	case ETTypePost:
		{
			...
			strPostGui.Format(_T("%s%s%d%s%s%s%s%s%s%s%d%s%s%s%s%s%s"), strPostGui.c_str(),
	...
	case ETypeDomainpost:
		{
			strPostGui.Format(_T("%s%s%d%s%s%s%d%s%s%s%s%s%d"),
	...
	case ETypeWebopost:
		{
			strPostGui.Format(_T("%s%s%d%s%s%s%d%s%s%s%s%s%d%s%s%s%s"),
	...
	case ETypeCompanyPopular:
		{
 			strPostGui.Format(_T("%s%s%d%s%s%s%s%s%s%s%d%s%s%s%s"),
	...
	case ETypeMailCount:
		{
			strPostGui.Format(_T("%s%s%d%s%s%s%s%s%d%s%s%s%s%s%s"),
	...
	case ETTypeUserInfoMigrate:
		{
			strPostGui.Format(_T("%s%s%d%s%s%s%d%s%s%s%d%s%d%s%s%s%s"),
	...
	case ETTypeModify:
		{
			strPostGui.Format(_T("%s%s%d%s%s%s%d%s%s%s%s%s%s"),
	...
	case ETTypeGetPdtList:
		{
			strPostGui.Format(_T("%s%s%d%s%s%s%s%s%d%s%s%s%s"),
	...
	case ETTypeSoftPost:
	{
			strPostGui.Format(_T("%s%s%d%s%s%s%d%s%s%s%s"),
				_T("BackResult(;0)SoftPost"),
				SIMPLE_LIST_SPLIT, pTaskData->tTaskNetItem.dwWebId,				//网站编号
				SIMPLE_LIST_SPLIT, strWebName.c_str(),							//网站名称
				SIMPLE_LIST_SPLIT, pTaskResultInfo->eResultFlag,				//任务结果
				SIMPLE_LIST_SPLIT, pTaskResultInfo->strResultInfo.c_str(),		//失败原因
