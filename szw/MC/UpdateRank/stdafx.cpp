...
TCHAR tSpecialChar[HtmlCharNum][CHAR_LEN] = { _T(" "), _T("+"), _T("/"), _T("?"), _T("%"), _T("#"), _T("&"), _T("="), _T("."), _T("\\"),_T(":"), _T("*"), _T("\""), _T("<"), _T(">"), _T("|") };
// 初始化map
void InitMap()
{
...
void ParseKeyWordData(const CStdString &strKeyWord)
{
	...
	// 关键词数据:
	// 关键词信息(;9)分割符列表(;9)搜索引擎名称(;9)网站列表(;9)目标网站标记(;9)公司名(;3)公司简称(;9)官网列表(;9)发布标记前缀
	// 分解关键词信息
	...
	if (iCountInfo < 6)
	{
		g_pageLog.Trace(LOGL_TOP, LOGT_WARNING, __TFILE__, __LINE__, _T("分解单条关键词数据失败，(;9)组数:%d"),iCountInfo);
		return;
	}
	...
	else if ((strWebFlag == _T("3")) || (strWebFlag == _T("4")))  //增加官网和排除列表（3）  //从默认列表排除制定网站(4)
	{
		...
		if (strWebFlag == _T("3"))
		{	
			//g_pageLog.Trace(LOGL_TOP, LOGT_PROMPT, __TFILE__, __LINE__, _T("收到的官网列表：%s"), strWebHost.c_str());
			vecWebList.clear();
