...
CInternetHttp::CInternetHttp(LPCTSTR strAgent)
{
	...
	PathAppend(szIniFile, _T("data2\\mc.dat"));

	GetPrivateProfileString(_T("UPDATE"), _T("http"), NULL, szInfo, MAX_PATH, szIniFile);
	...
	g_pageLog.Trace(LOGL_TOP, LOGT_PROMPT, __TFILE__, __LINE__, _T("http登录地址：%s, 上传地址：%s"), strUrlHome.GetString(), strUrlUpload.GetString());
...
CString CInternetHttp::MakePostFileData(const CString &strBoundary, const CString strFilePath)
{
	...
	strFormat += _T("BusinessExpressImage/");
	strFormat += _T("\r\n");
...
BOOL CInternetHttp::LoginAndUploadFile( const CString& strFileLocalFullPath )
{
	...
	if (strResponse.GetLength() > 0)
	{
		...
		{
			g_pageLog.Trace(LOGL_TOP, LOGT_PROMPT, __TFILE__, __LINE__, _T("http上传快照成功,返回响应文本为：%s"), strResponse.GetString());
			return TRUE;
		}
		else
		{
			g_pageLog.Trace(LOGL_TOP, LOGT_WARNING, __TFILE__, __LINE__, _T("http上传快照失败,返回响应文本为：%s"), strResponse.GetString());
		}
	}
	else
	{
		g_pageLog.Trace(LOGL_TOP, LOGT_ERROR, __TFILE__, __LINE__, _T("http上传时打开页面打败"));
	}
