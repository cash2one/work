...
typedef  IThreadUnit *(*CREATEITHREAD)(IGlobalDataBlock* pFindDataByID, IServerSocket *pSocket, IXMLRW *pConfig);
...
DWORD  CMCCtrl::Start(BOOL bYun)
{
	...
	for(int i=0;i<10;i++)
	{
		...
		int nCount = 0;
		g_Config->ReadInt(_T("MC"), strField, _T("count"), nCount);
		g_Config->ReadString(_T("MC"), strField, _T("file"), strTmp);
		strFile = strTmp;
