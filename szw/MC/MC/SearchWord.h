...
public:
	...
	//接口，处理淘宝字段
	void ProcessTBWord(const CStdString &strWord);

private:
	//解析搜索关键词数据
	BOOL ParseSearchKeyWordData(const CStdString &strData, std::vector<CStdString> &vStrWords);
	//打开URL获取源
	...
	//获取淘宝网页查询到的数据
	BOOL GetTBData(const CString &strData, const CString &strKey);

	//替换网页一些不用的数据
	void ReplacePara(CString &strData);
	//检查是否在我们的商贸网站中
