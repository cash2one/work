...
DWORD CTaskMgrThread::RevExSerResult(DWORD dwIndex)
{
	...
	if (m_dwFinishCount == m_dwTotalTaskCount-1)
	{
		TaskResult(dwIndex,TRUE);
	}
	else
	{
		TaskResult(dwIndex);
...
DWORD CTaskMgrThread::DispatchMessage(T_Message *Msg)
{
    ...
    case E_PPD_CONNDBOK_MSG: 
        {
            m_DBState = e_open;
			g_log.Trace(LOGL_TOP, LOGT_PROMPT,__TFILE__, __LINE__, _T("数据准备初始化完成\n"));
            //printf("数据准备初始化完成\n");
	...
	case MSG_TIMER:
		{
			g_log.Trace(LOGL_TOP, LOGT_PROMPT,__TFILE__, __LINE__,_T("所剩任务总数 = %05d 当前正在做的任务 = %5d 任务完成数 = %5d"),m_TaskDescMap.size(),m_TaskDoingCount,m_dwFinishCount);
		...
		&& bIsSendComplete == FALSE)
	{
		...
		if (pLocalGlobalData->bIsYunTask)
		{
			PostUrlAccountData(FALSE);
		}
		CString strMac = GetPhysicAddr();
		g_log.Trace(LOGL_TOP, LOGT_PROMPT,__TFILE__, __LINE__,_T("Mac地址：%s"),strMac.GetBuffer(strMac.GetLength()));
		CStdString strBack;
		...
		g_log.Trace(LOGL_TOP, LOGT_PROMPT,__TFILE__, __LINE__,_T("所有任务结束，主控退出"));
...
DWORD CTaskMgrThread::RevPrepareNoWebData(DWORD dwIndex)
{
	...
	g_log.Trace(LOGL_TOP, LOGT_PROMPT, __TFILE__,__LINE__, _T("网站需要更新数据 taskid:%d"),dwIndex);
    return 0;
}
//数据准备发来网站数据更新完成
DWORD CTaskMgrThread::RevPrepareWebUpdateSucceed(DWORD dwIndex)
{
	...
	g_log.Trace(LOGL_TOP, LOGT_PROMPT, __TFILE__,__LINE__, _T("网站更新数据完成 taskid:%d"),dwIndex);
...
DWORD CTaskMgrThread::RevPrepareWebUpdateFailed(DWORD dwIndex)
{
	...
	g_log.Trace(LOGL_TOP, LOGT_ERROR, __TFILE__,__LINE__, _T("网站更新数据失败 taskid:%d"),dwIndex);
    return 0;
}
...
DWORD CTaskMgrThread::PushNormalTaskData(DWORD dwGUIDataIndex)
{
	...
	if (pLocalGlobalData != NULL)
	{
		...
		{
			g_log.Trace(LOGL_TOP, LOGT_WARNING, __TFILE__,__LINE__, _T("网站数据还未准备好。"));
		}
	}
