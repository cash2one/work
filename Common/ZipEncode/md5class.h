...
public:
	...
	void setBinaryData(unsigned char* binarydata,int dwLen); //set plaintext with a mutator, it's ok to 
...
private:
	...
	bool calcDigestB(); //this function computes the digest by calling the RFC 1321 'C' code
	...
	unsigned char* m_binary; //a pointer to the plain text.  If casting away the const-ness 
        int dwDataLen;  //binaryData Length
