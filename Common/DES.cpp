...
//加密
void CDES::Encrypt( CString &strOut,CString strIn,CString strKey )
{
	char hexCiphertextAnyLength[16384] = {0};
	char bitsCiphertextAnyLength[32768] = {0};
	char Key[8] = {0};
	char szEncryptData[8192] = {0};

	int i = strKey.GetLength();
	int iChar = WideCharToMultiByte(CP_ACP,0,strKey,i,NULL,0,NULL,NULL);//按Byte计算str长度
	char *szKey = new char[iChar + 1];
	WideCharToMultiByte(CP_ACP,0,strKey,i,szKey,iChar,NULL,NULL);//
	szKey[iChar] = '\0';
	memcpy(Key,szKey,strlen(szKey) < 8 ? strlen(szKey) : 8);

	i = strIn.GetLength();
	iChar = WideCharToMultiByte(CP_ACP,0,strIn,i,NULL,0,NULL,NULL);//按Byte计算str长度
	char *szEnData = new char[iChar + 1];
	WideCharToMultiByte(CP_ACP,0,strIn,i,szEnData,iChar,NULL,NULL);//
	szEnData[iChar] = '\0';

	memcpy(szEncryptData,szEnData,strlen(szEnData));

	InitializeKey(Key,0);

	EncryptAnyLength(szEncryptData,strlen(szEncryptData),0);

	int iBitsLen = strlen(szEncryptData) % 8 == 0 ? strlen(szEncryptData) << 3 : ((strlen(szEncryptData)>>3) + 1) << 6;

	//	ConvertCiphertext2OtherFormat(strlen(szEncryptData) % 8 == 0 ? strlen(szEncryptData) << 3 : ((strlen(szEncryptData)>>3) + 1) << 6,GetCiphertextAnyLength());

	memset(hexCiphertextAnyLength,0,16384);
	memset(bitsCiphertextAnyLength,0,32768);
	Bytes2Bits(GetCiphertextAnyLength(),bitsCiphertextAnyLength,iBitsLen);
	Bits2Hex(hexCiphertextAnyLength,bitsCiphertextAnyLength,iBitsLen);

	strOut = hexCiphertextAnyLength;
...
//解密
void CDES::Decrypt( CString &strEncryptData,CString strDecryptData,CString strKey )
{
	char hexCiphertextAnyLength[16384] = {0};
	char bitsCiphertextAnyLength[32768] = {0};
	char Key[8] = {0};
	char szDecryptData[8192] = {0};
	...
	int iLen = 0;
	i = strDecryptData.GetLength();
	iChar = WideCharToMultiByte(CP_ACP,0,strDecryptData,i,NULL,0,NULL,NULL);//按Byte计算str长度
	char *szCipher = new char[iChar + 1];
	WideCharToMultiByte(CP_ACP,0,strDecryptData,i,szCipher,iChar,NULL,NULL);//
	szCipher[iChar] = '\0';
	...
	Bits2Bytes(szDecryptData,bitsCiphertextAnyLength,iLen);

	iLen = iLen>>3;
	DecryptAnyLength(szDecryptData,iLen,0);

	strEncryptData = GetPlaintextAnyLength();
