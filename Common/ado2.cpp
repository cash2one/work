...
#else
	g_log.Trace(LOGL_HIG, LOGC_ERROR, __TFILE__, __LINE__, ErrorStr) ;
	#endif
...
{	
	...
	m_strQuery.TrimLeft();
	BOOL bIsSelect = m_strQuery.Mid(0, _tcsclen(_T("Select "))).CompareNoCase(_T("select ")) == 0 && nOption == openUnknown;
...
{	
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			//val = val /(double)(pow(10 , vtFld.decVal.scale)) ;
			tempScale = vtFld.decVal.scale;
			val /= pow((long double)10, tempScale); 
			break;
		case VT_UI1:
			val = vtFld.iVal;
			break;
...
BOOL CADODatabase::Execute(LPCTSTR lpstrExec)
{
	...
	{
		MessageBox(NULL, _T("数据库出现错误异常"), _T(""), 0);
		dump_com_error(e);
		return FALSE;	
	}
}
...
BOOL CADORecordset::GetFieldInfo(FieldPtr pField, CADOFieldInfo* fldInfo)
{
	...
	strcpy(fldInfo->m_strName, (LPCSTR)pField->GetName());
...
BOOL CADOParameter::GetValue(double& dbValue)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			nTempScale = vtVal.decVal.scale ;
			dblVal /= pow((long double)10, nTempScale); 
			break;
		case VT_UI1:
			dblVal = vtVal.iVal;
			break;
